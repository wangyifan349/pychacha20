...
A compact, pure-Rust implementation of **ChaCha20-Poly1305 AEAD** based on **RFC 8439**, with an interactive CLI for encrypting or decrypting a single file or an entire directory.

The program uses only Rust's standard library. It keeps file names and directory structure unchanged and generates a fresh 96-bit nonce for every encrypted file.
...
```bash
target/release/chacha20_poly1305
```

On Windows:

```text
target\release\chacha20_poly1305.exe
```
...
- Before replacing a file, the current implementation writes to a temporary file and then replaces the original file.
- Pure Rust is useful for learning, interoperability testing, and studying the construction, but it does **not** provide the constant-time guarantees or hardening expected from production cryptographic libraries.
- A nonce must never be reused with the same key. The CLI avoids manual nonce reuse by generating a new 12-byte nonce for each encryption operation.
...
## 📜 License
The `rs/chacha20ploy1305.rs` file in this repository is licensed under the **Apache License 2.0**.
